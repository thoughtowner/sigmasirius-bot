...
from src.commands import REPAIRMAN_STATS
from aio_pika.exceptions import QueueEmpty
import asyncio

@router.message(F.text == REPAIRMAN_STATS)
async def repairman_stats(message: Message):
    ...
    async with channel_pool.acquire() as channel:
        user_queue = await channel.declare_queue(
            settings.USER_CHECK_START_QUEUE_TEMPLATE.format(telegram_id=telegram_id),
            durable=True,
        )
