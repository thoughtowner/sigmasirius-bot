...
async def handle_become_repairman_event(message): # TODO async def handle_become_repairman_event(message: RepairmanMessage)
    async with async_session() as db:
        ...
        if not message['is_test_data']:
            buf = io.BytesIO()
            img = qrcode.make('repairman/' + str(user_id))
            img.save(buf, format='PNG')
            buf.seek(0)
            image_file = BufferedInputFile(file=buf.read(), filename='become_repairman_qr.png')
            
            # import bot lazily to avoid circular import at module import time
            from src.bot import bot
            await bot.send_photo(
                chat_id=message['telegram_id'], photo=image_file,
                caption='Ваш QR-код для получения должности ремонтника. Покажите его на ресепшене.',
            )
